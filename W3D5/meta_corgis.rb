...
class MetaCorgiSnacks
  def initialize(snack_box, box_id)
    @snack_box = snack_box
    @box_id = box_id
    snack_box.methods.grep(/^get_(.*)info$/) { MetaCorgiSnacks.define_snack $1}
  end

  # def method_missing(name, *args)
  #   info = @snack_box.send("get_#{name}_info", @box_id)
  #   tastiness = @snack_box.send("get_#{name}_tastiness", @box_id)
  #   show_name = "#{name}".split("_").join(" ")
  #   results = "#{show_name}:#{info} #{tastiness}"
  #   tastiness > 30 ? "* #{results}" : results
  #
  #
  # end
